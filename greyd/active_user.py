...
class ActiveUser(DatabaseGreyd):
    ...
    def __game_active_user__(self, json_request):
        ...
        with sql.connect(self.db_path) as database:
            ...
            lobby_status = cursor.execute("""SELECT lobby_status
                                          FROM lobbies
                                          WHERE lobby_id=?""",
                                          (lobby_id,)).fetchone()
            ...
            user_bait = cursor.execute("""SELECT count(*)
                                       FROM user_location
                                       WHERE session_id=?""",
                                       (session_id,)).fetchone()
    ...
    def __refresh_lobby__(self, json_request):
        ...
        with sql.connect(self.db_path) as database:
            ...
            response["users"] = self.__user_info_same_lobby__(lobby_id,
                                                              session_id,
                                                              last_seen_chat_id)
    ...
    def __user_info_same_lobby__(self, lobby_id, session_id, last_seen_chat_id):
        ...
        with sql.connect(self.db_path) as database:
            ...
            for user in users:
                ...
                user_chat = []
                user_info = {"userFullName": user[0],
                             "userGreydId": user[1],
                             "userFacebookId": user[2],
                             "userScore": user[3],
                             "userLocation": user[4],
